...
From Bythos.Protocols.PB Require Export Safety.
...
Definition all_echoes src r w : Prop := forall n, isByz n = false -> (w @ n).(echoed) (src, r) = Some (value_bft src r).
...
Section Proof_of_Termination.
  ...
  Proof.
    ...
    exists (List.filter (fun p =>
      ...
      (Address_eqdec (P0.src p) src) && (negb (isByz p.(dst)))) (packetSoup w)).
  ...
  Definition round_1_end w' := Eval unfold initmsg_in in
    forall n, isByz n = false -> initmsg_in n true (packetSoup w').
  ...
  Proof.
    ...
    exists (List.filter (fun p => 
      match p.(msg) with
      | EchoMsg _ _ => (negb (isByz (P0.src p))) && (negb (isByz p.(dst)))
      | _ => false
      end) (packetSoup w)).
    split_and?; auto using incl_filter.
  ...
  Definition round_2_end w' := Eval unfold echomsg_in in
    forall n, isByz n = false -> echomsg_in n true (packetSoup w').
  ...
  Lemma src_outputs_suffcond w0 l0 (Htrace0 : system_trace w l0) (Ew0 : w0 = final_sysstate w l0) 
  ...
  Proof.
    ...
    assert (incl (List.filter (fun n => negb (isByz n)) valid_nodes) (map fst ((w0 @ src).(counter) r))) as Hincl.
